    ...
    renderForecastChart(result) {
      ...
      this.forecastChart = new Chart(canvas, {
        ...
        data: {
          ...
          datasets: [{ data: result.daily.map(day => day.total), borderColor: accent, backgroundColor: accent, fill: { target: 'origin', above: this.hexToRgba(accent, 0.1), below: this.hexToRgba(accent, 0.1) }, tension: 0.3 }]
        },
        options: {
          responsive: true,
          ...
          plugins: {
            ...
            title: { display: false },
            tooltip: { usePointStyle: true, boxPadding: 3, callbacks: { label: context => ' Projected balance: ' + this.fmtBase(context.parsed.y) } }
          },
          scales: {
            x: { ticks: { color: '#888888', maxRotation: 0, autoSkip: false, maxTicksLimit: 8, font: { size: 11 } }, grid: { color: '#2a2a2a', drawBorder: false }, border: { display: false } },
            y: { ticks: { color: '#888888', font: { size: 11 }, maxTicksLimit: 5, callback: value => this.fmtNumber(value) }, grid: { color: '#2a2a2a', drawBorder: false }, border: { display: false } }
          }
        }
      });
    },
