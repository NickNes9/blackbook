...
export function createProfileStore({ profilesDir, legacyDataFile, defaultData }) {
  ...
  const authFile = join(profilesDir, 'auth');
  ...
  function readAuth() {
    try { return JSON.parse(readFileSync(authFile, 'utf8')); }
    catch (_) { return {}; }
  }

  function writeAuth(map) {
    writeFileSync(authFile, JSON.stringify(map, null, 2), 'utf8');
  }
  ...
  return {
    ...
    rename(name, newName) {
      ...
      const auth = readAuth();
      if (Object.hasOwn(auth, clean)) {
        auth[cleanNew] = auth[clean];
        delete auth[clean];
        writeAuth(auth);
      }
    },
    ...
    hasPassword(profile) {
      const name = safeProfileName(profile);
      return name === null ? false : Object.hasOwn(readAuth(), name);
    },
    getAuth(profile) {
      const name = safeProfileName(profile);
      return name === null ? null : (readAuth()[name] || null);
    },
    setAuth(profile, entry) {
      const name = safeProfileName(profile);
      if (name === null) throw new StorageError('Invalid profile name', 400);
      const auth = readAuth();
      auth[name] = entry;
